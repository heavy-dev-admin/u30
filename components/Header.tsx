...
export default function Header({ settings }: { settings: Settings }) {
  ...
  return (
    <header className="bg-cream p-4 relative md:px-6.5 md:pt-6.5 md:pb-4">
      ...
      <div
        className={`absolute top-0 left-0 w-full h-dvh bg-cream z-10 p-4 transition-all duration-300 ease-in-out md:hidden ${isMenuOpen ? 'opacity-100 translate-x-0 pointer-events-auto' : 'opacity-0 -translate-x-full pointer-events-none'}`}
      >
